        ...
        >
          <SortableContext items={items} strategy={rectSwappingStrategy}>
            {items.map((item, index) => (
              <Fragment key={index}>
                <SortableItem as={'div'} data={item} id={item.id} className={cn('relative h-full group', item.className)}>
                  {item.content}
                  {enabled && (
                    <DragHandle
                      className={cn(
            ...
            {activeItem && (
              <DragOverlay>
                <Box className={'[&_>div]:shadow-xl dark:[&_>div]:shadow-md dark:[&_>div]:shadow-gray-100'}>{activeItem.content}</Box>
              </DragOverlay>
            )}
          </SortableContext>
        </DndContext>
